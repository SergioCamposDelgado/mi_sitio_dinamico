...
final class ProductoController
{
    ...
    public static function crear(): void
    {
        ...
        if ($nombre === '' || !is_numeric($precio) || (float)$precio < 0 || !is_numeric($stock) || (int)$stock < 0 || strlen($descripcion) > 120) {
            http_response_code(422);
            echo self::error('Datos inválidos', 'index.php?p=productos&action=nuevo');
            return;
        }
        ...
        header('Location: index.php?p=contenido');
        exit;
    }
    ...
    public static function actualizar(): void
    {
        ...
        if ($id <= 0 || $nombre === '' || !is_numeric($precio) || (float)$precio < 0 || !is_numeric($stock) || (int)$stock < 0 || strlen($descripcion) > 120) {
            http_response_code(422);
            echo self::error('Datos inválidos', 'index.php?p=contenido');
            return;
        }

        $pdo = Database::getConnection();
        $dao = new ProductoDAO($pdo);
        $p   = $p   = new Producto($nombre, (float)$precio, (int)$stock, $descripcion);
        $p->setId($id);
